...
const Applications = () => {
  ...
  const [filters, setFilters] = useState({ search: '', status: 'All', jobType: 'All', sort: 'newest' });
    ...
    <div className="page-stack">
      ...
      <section className="filters-panel">
        <label className="search-field">
          <Search size={18} />
          <input name="search" value={filters.search} onChange={updateFilter} placeholder="Search company, role, location..." />
        </label>
        ...
        <select name="jobType" value={filters.jobType} onChange={updateFilter} aria-label="Filter by job type">
          <option>All</option>
          {JOB_TYPES.map((type) => (
            <option key={type}>{type}</option>
          ))}
        </select>
        <select name="sort" value={filters.sort} onChange={updateFilter} aria-label="Sort applications">
